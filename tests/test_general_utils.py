...
class TestEnhanceTitle:
    ...
    def test_preserves_leading_roman_numeral(self):
        assert enhance_title("IV THE BEGINNING") == "IV The Beginning"

    def test_no_change_for_mixed_case(self):
        assert enhance_title("Hello World") == "Hello World"

    def test_fixes_possessive_after_title_case(self):
...
class TestNormalizeLigatures:
    ...
    def test_normalizes_ffi_ligature(self):
        assert normalize_ligatures("ﬃcient") == "fficient"

    def test_normalizes_ffl_ligature(self):
        assert normalize_ligatures("ﬄuent") == "ffluent"

    def test_normalizes_st_ligature(self):
...
class TestFixEncodingArtifacts:
    ...
    def test_fixes_right_double_quote(self):
        assert fix_encoding_artifacts("helloÓ") == 'hello"'

    def test_fixes_apostrophe(self):
...
class TestFixBracketSpacing:
    ...
    def test_removes_space_inside_square_brackets(self):
        assert fix_bracket_spacing("[ hello ]") == "[hello]"

    def test_removes_space_inside_curly_braces(self):
        assert fix_bracket_spacing("{ hello }") == "{hello}"
...
class TestFixApostrophes:
    ...
    def test_fixes_space_before_possessive(self):
        assert fix_apostrophes("the dog 's bone") == "the dog's bone"
...
class TestStripFootnoteNumbers:
    ...
    def test_strips_multiple_trailing_digits(self):
        assert strip_footnote_numbers("Hello world.123") == "Hello world."

    def test_no_change_when_ends_with_punctuation(self):
...
class TestLoadSectionsToSkip:
    def test_loads_sections_from_csv(self, tmp_path):
    ...
    def test_handles_multiple_books(self, tmp_path):
        csv_path = tmp_path / "sections_to_skip.csv"
        csv_path.write_text(
